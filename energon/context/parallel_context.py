...
from energon.constants import ALLOWED_MODES, INITIALIZER_MAPPING, TENSOR_PARALLEL_MODE
from energon.context.config import Config
from energon.logging import get_dist_logger
from energon.registry import DIST_GROUP_INITIALIZER

from .parallel_mode import ParallelMode
from .random import add_seed, get_seeds, set_mode
# from energon.global_variables import moe_env
...
class ParallelContext:
    ...
    def rm_local_rank(self, parallel_mode: ParallelMode):
        ...
        """
        self._check_parallel_mode(parallel_mode)
        self._local_ranks.pop(parallel_mode)
    ...
    def rm_world_size(self, parallel_mode: ParallelMode):
        """Removes world size for `parallel_mode`.
        """
        self._check_parallel_mode(parallel_mode)
        self._world_sizes.pop(parallel_mode)
    ...
    def rm_group(self, parallel_mode: ParallelMode):
        """Removes the group of the current device for `parallel_mode`.
        """
        self._check_parallel_mode(parallel_mode)
    ...
    def _deregister_dist(self, mode):
        self.rm_local_rank(mode)
        self.rm_world_size(mode)
        self.rm_group(mode)
        self.rm_ranks_in_group(mode)
    ...
    def init_parallel_groups(self):
        ...
        :raises AssertionError: Raises an AssertionError if the field parallel is not present in the config file
        """
