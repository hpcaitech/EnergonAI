...
template <typename T>
__global__ void ker_scale_mask_softmax(T* correlation,
...
template void ker_scale_mask_softmax_launcher<float>(
    int batch_size, int batch_seq_len, int head_num,
    float* correlation, const int* real_seq_len);

template void ker_scale_mask_softmax_launcher<__half>(
    int batch_size, int batch_seq_len, int head_num,
