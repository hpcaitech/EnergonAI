...
store_glb2mem_trans_conflict(const T* A, \
                const int A_offset, \
                const T* B, \
                const int B_offset, \
                T* s_a_buf, \
                T* s_b_buf,
                const int M, const int N, const int K, const int w){
    int xw = threadIdx.x*w;
    int yN = threadIdx.y * N;

    s_a_buf[threadIdx.y + xw] = A[A_offset + threadIdx.x + yN];
    s_b_buf[threadIdx.y + xw] = B[B_offset + threadIdx.x + yN];
}


template<typename T>
static __global__ void
fine_indexed_mm(const T* A, \
                const int* A_offset, \
                const T* B, \
                const int* B_offset, \
                T* C, \
        ...
        T* s_a_buf = (T*) array;
        T* s_b_buf = (T*) array + w*w;

        int bid = blockIdx.x;
        
        store_glb2mem_trans_conflict<T>(A, A_offset[bid], B, B_offset[bid], s_a_buf, s_b_buf, M, N, K, w);
        __syncthreads();
        ...
        for(int i=0; i<w; i++){
            int wi = w*i;
            tmp += s_a_buf[wi + threadIdx.x] * s_b_buf[wi + threadIdx.y];
            // tmp += __hmul(s_a_buf[wi + threadIdx.x], s_b_buf[wi + threadIdx.y]);
        }
        
        int offset = C_offset[bid] + threadIdx.y*M + threadIdx.x;
        
        atomicAdd((C+offset), tmp);
    }

// recommend m = n = k = 32
template<typename T>
void fineQK_Kernel_2D(const T* A, \
                            const int* A_offset, \
                            const T* B, \
                            const int* B_offset, \
                            T* C, \
        ...
        dim3 dimBlock(w, w);
        dim3 dimGrid(num);
        int sharemem_size = w * w * sizeof(T) * 2;

        // cudaMemset(C, 0, 32*12*128*128*sizeof(T));
        // cudaDeviceSynchronize();
        
        fine_indexed_mm<T><<<dimGrid, dimBlock, sharemem_size, stream>>>(A,
                A_offset, B, B_offset, C, C_offset, M, N, K, w);
    }

template void fineQK_Kernel_2D<float>(const float* A, \
                            const int* A_offset, \
                            const float* B, \
                            const int* B_offset, \
                            float* C, \
                            ...
                            const int M, const int N, const int K, const int w, \
                            cudaStream_t stream);
template void fineQK_Kernel_2D<half>(const half* A, \
                            const int* A_offset, \
                            const half* B, \
                            const int* B_offset, \
                            half* C, \
...
template<typename T>
static __device__ __inline__ void
store_glb2mem_trans_conflict_1d(const T* A, \
                const int A_offset, \
                const T* B, \
                const int B_offset, \
                T* s_a_buf, \
                ...
                const int width){
    // here only process size_per_head // width = 2                  
    int xw = threadIdx.x*width;
    int ys = threadIdx.y*size_per_head;
    int stride = width*width;

    s_a_buf[threadIdx.y + xw] = A[A_offset + threadIdx.x + ys];
    s_b_buf[threadIdx.y + xw] = B[B_offset + threadIdx.x + ys];

    
    s_a_buf[threadIdx.y + xw + stride] = A[A_offset + threadIdx.x + ys + width];
    s_b_buf[threadIdx.y + xw + stride] = B[B_offset + threadIdx.x + ys + width];
}
...
fine_indexed_mm_1d(const T* A, \
        ...
        extern __shared__ float array[];
        T* s_a_buf = (T*) array;
        T* s_b_buf = (T*) array + size_per_head * width;

        int bid = blockIdx.x;
        
        store_glb2mem_trans_conflict_1d<T>(A, A_offset[bid], B, B_offset[bid], s_a_buf, s_b_buf, size_per_head, width);
        __syncthreads();
        ...
        {
            int index = i*32;
            tmp += s_a_buf[index + threadIdx.x] * s_b_buf[index + threadIdx.y]; 
        }   
        ...
        int offset = C_offset[bid] + threadIdx.y*width + threadIdx.x;
        C[offset] = tmp;
...
template<typename T>
void fineQK_Kernel_1D(const T* A, \
                      const int* A_offset, \
                      const T* B, \
                      const int* B_offset, \
                      T* C, \
  ...
  dim3 dimGrid(num);
  int sharemem_size = width * size_per_head * sizeof(T) * 2;
  fine_indexed_mm_1d<<<dimGrid, dimBlock, sharemem_size, stream>>>(A, A_offset, B, B_offset, C, C_offset, size_per_head, width);

}

template void fineQK_Kernel_1D<float>(const float* A, \
                      const int* A_offset, \
                      const float* B, \
                      const int* B_offset, \
                      float* C, \
...
template void fineQK_Kernel_1D<half>(const half* A, \
                      const int* A_offset, \
                      const half* B, \
                      const int* B_offset, \
                      half* C, \
...
template<typename T>
static __device__ __inline__ void
store_glb2mem_trans_conflict2(const T* A, \
                const int A_offset, \
                const T* B, \
                const int B_offset, \
                T* s_a_buf, \
                T* s_b_buf,
                const int M, const int N, const int K, const int w){
    s_a_buf[threadIdx.y + threadIdx.x * w] = A[A_offset + threadIdx.x + threadIdx.y * M];
    s_b_buf[threadIdx.x + threadIdx.y * w] = B[B_offset + threadIdx.x + threadIdx.y * K];
}
...
fine_indexed_mm2(const T* A, \
        ...
        store_glb2mem_trans_conflict2(A, A_offset[bid], B, B_offset[bid], s_a_buf, s_b_buf, M, N, K, w);
        __syncthreads();
        ...
        int offset = C_offset[bid] + threadIdx.y + threadIdx.x* K;
        atomicAdd((C+offset), tmp);
}


template<typename T>
void fineWV_Kernel_2D(const T* A, \
                            const int* A_offset, \
                            const T* B, \
                            const int* B_offset, \
                            T* C, \
        ...
        // cudaMemset(C, 0, 32*12*64*128*sizeof(T));
        // cudaDeviceSynchronize();
        fine_indexed_mm2<T><<<dimGrid, dimBlock, sharemem_size, stream>>>(A,
                A_offset, B, B_offset, C, C_offset, M, N, K, w);
}

template void fineWV_Kernel_2D<float>(const float* A, \
                            const int* A_offset, \
                            const float* B, \
                            const int* B_offset, \
                            float* C, \
...
template void fineWV_Kernel_2D<half>(const half* A, \
                            const int* A_offset, \
                            const half* B, \
                            const int* B_offset, \
                            half* C, \
