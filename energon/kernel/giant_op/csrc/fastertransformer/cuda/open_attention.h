...
// #endif
      ...
      const DataType_ scalar)
  {
    ...
    {
      // printf("multiHeadAttr_nofuse_kernelLauncher \n");
      // if(param_.sequence_id_offset == nullptr || param_.valid_word_num == batch_size * seq_len)
      if(param_.padding_strategy == 0 || param_.padding_strategy == 2)
      {
      ...
      else if(param_.padding_strategy == 1)
      {
      ...
      // 从这里开始
      // printf("Step0");   
      if(param_.padding_strategy == 1 || param_.padding_strategy == 0)
      {
      ...
      }else if(param_.padding_strategy == 2){  // 32 is the width, a preset num.
        int width = 32;
        // printf("before qk kernel");
        // fineQK_Kernel_2D<float>(q_buf_, param_.d_q_offset, k_buf_, param_.d_k_offset, qk_buf_, param_.d_w_qk_offset, param_.h_valid_num[0], seq_len, size_per_head, seq_len, width, stream);
        fineQK_Kernel_1D<float>(q_buf_, param_.d_q_offset, k_buf_, param_.d_k_offset, qk_buf_, param_.d_w_qk_offset, param_.h_valid_num[0], size_per_head, width, stream);
        
        // printf("after qk kernel");
        attn_softmax_kernelLauncher(qk_buf_, attr_mask, batch_size, seq_len, head_num, scalar, stream);
        // // printf("before wv kernel");
        fineWV_Kernel_2D<float>(qk_buf_, param_.d_w_wv_offset, v_buf_, param_.d_v_offset, transpose_dst_, param_.d_r_offset, param_.h_valid_num[1], seq_len, seq_len, size_per_head, width, stream);
        // printf("after wv kernel \n");
      }
  ...
  void forward(const DataType_* from_tensor, const DataType_* to_tensor)
  {
    ...
    int m = 0;
    if(param_.padding_strategy == 0)
    {
      m = batch_size_ * from_seq_len_;
    }else if(param_.padding_strategy == 1 || param_.padding_strategy == 2){
      m = param_.valid_word_num;
    }
