...
def get_dist_logger(name='energon'):
    ...
    :rtype: :class:`energon.logging.DistributedLogger`
    """
    return DistributedLogger.get_instance(name=name)


def disable_existing_loggers(except_loggers: List[str] = ['energon']):
    """Set the level of existing loggers to `WARNING`.

    :param except_loggers: loggers in this `list` will be ignored when disabling, defaults to ['energon']
