    ...
    def forward(self, 
        ...
        for _ in range(cur_len, tgt_len):
            ...
            if attention_mask is not None:
                attention_mask_unfold = attention_mask.view(batch_size, -1)
                attention_mask_unfold = attention_mask_unfold.unsqueeze(1).unsqueeze(2)
                attention_mask_unfold = attention_mask_unfold.to(dtype=hidden_states.dtype)  # fp16 compatibility
                attention_mask_unfold = (1.0 - attention_mask_unfold) * -10000.0

            for block in self.blocks:
                hidden_states = block(hidden_states=hidden_states,
                                    attention_mask=attention_mask_unfold,
                                    ) # tgt_len=tgt_len
