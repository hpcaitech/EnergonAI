...
from energonai.communication import broadcast
from colossalai.core import global_context as gpc
from colossalai.context import ParallelMode, seed
from energonai.nn import init as init
...
from energonai.utils.checkpointing import partition_tensor_parallel_state_dict, gather_tensor_parallel_state_dict, \
...
class MixedFusedLayerNorm1D(ParallelLayer):
    r"""
    ...
    def _save_to_global_state_dict(self, destination, prefix, keep_vars):
        if gpc.get_local_rank(ParallelMode.TENSOR) == 0:
            super(ParallelLayer, self)._save_to_state_dict(destination, prefix, keep_vars)
...
class VocabParallelClassifier1D(ParallelLayer):
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        output = gather_forward_split_backward(output, ParallelMode.PARALLEL_1D, -1)
        return output
