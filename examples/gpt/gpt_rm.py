...
from energon.kernel import ft_build_padding_offsets, ft_remove_padding, ft_rebuild_padding, ft_transpose_remove_padding, ft_transpose_rebuild_padding
...
class GPTSelfAttention1D(nn.Module):
    ...
    def forward(self, x, attention_mask=None, batch_size=None, max_padding_size=None, seq_lens=None, mask_offset=None, valid_word_num=None):
        ...
        if seq_lens is not None:
            ...
            # q_buf = torch.zeros(batch_size, num_attention_heads, max_padding_size, self.attention_head_size, dtype = q.dtype).cuda()
            # k_buf = torch.zeros(batch_size, num_attention_heads, max_padding_size, self.attention_head_size, dtype = q.dtype).cuda()
            # v_buf = torch.zeros(batch_size, num_attention_heads, max_padding_size, self.attention_head_size, dtype = q.dtype).cuda()
            # ft_transpose_rebuild_padding(q, k, v, q_buf, k_buf, v_buf, batch_size, max_padding_size, 
            #                         num_attention_heads, self.attention_head_size, valid_word_num[0].item(), mask_offset)
        else:
            qkv = qkv.permute((0, 2, 1, 3))
            q, k, v = torch.chunk(qkv, 3, dim=-1)

        # print(f'2: {q.size()}, {q[0]}')
        ...
        # print(f'3: {x.size()}, {x[0]}')
        # print(f'2: {x.size()}')

        if seq_lens is not None:
            # x = ft_transpose_remove_padding(x, valid_word_num[0].item(), batch_size, max_padding_size, 
            #                         num_attention_heads, self.attention_head_size, mask_offset)
...
class GPTBlock1D(nn.Module):
    ...
    def forward(self, x, attention_mask=None, batch_size=None, max_padding_size=None, seq_lens=None, mask_offset=None, valid_word_num=None):
        ...
        if self.apply_post_layernorm:
            residual = x
        x = residual + self.attn(x, attention_mask, batch_size, max_padding_size, seq_lens, mask_offset, valid_word_num)
...
class PipelineGPT1D(nn.Module):
    ...
    def forward(self, hidden_states=None, input_ids=None, attention_mask=None, seq_lens=None):
        ...
        if self.first:
            ...
            # if seq_lens is not None:
            #     hidden_states = depad(hidden_states, batch_size, seq_lens)
            if seq_lens is not None:
        ...
        for block in self.blocks:
            hidden_states = block(hidden_states, attention_mask, batch_size, max_padding_size, seq_lens, self.mask_offset, self.valid_word_num)
