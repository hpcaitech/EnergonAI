...
class PipelineGPT1D(nn.Module):
    ...
    def __init__(self,
        ...
        for id_ in range(depth): # layer name rule is wrong for load balanced pipeline
    ...
    def select_top_k(self, batch_id, temp_predictions, top_k: int = 10):
        ...
        TODO: Here the select topk method is wrong for batch condition.
        """
...
def partition_uniform(num_items, pipeline_parallel_size, num_chunks):
    ...
    parts = [[] for _ in range(pipeline_parallel_size)]  # 4 list(list,list,list,list)
...
def partition_lb(depth, pipeline_size, num_redundant_layers):
    assert depth % pipeline_size == 0, \
        "Depth should be divided by the number of pipeline stage, otherwise parameter method is recomended"
    parts = [[] for _ in range(pipeline_size)]
    chunk_size = depth // pipeline_size
      
    base_idx = 0    
    for p in range(pipeline_size):
        st = base_idx
        base_idx += chunk_size
        
        if(st == 0):
            parts[p].append((st, base_idx - num_redundant_layers))
            parts[p].append((base_idx - num_redundant_layers, base_idx))
            parts[p].append((base_idx, base_idx + num_redundant_layers))
        elif(base_idx == depth):
            parts[p].append((st - num_redundant_layers, st))
            parts[p].append((st, st + num_redundant_layers))
            parts[p].append((st + num_redundant_layers, base_idx))
        else:
            parts[p].append((st - num_redundant_layers, st))
            parts[p].append((st, st + num_redundant_layers))
            parts[p].append((st + num_redundant_layers, base_idx - num_redundant_layers))
            parts[p].append((base_idx - num_redundant_layers, base_idx))
            parts[p].append((base_idx, base_idx + num_redundant_layers))
    return parts


def _create_lb_gpt_pipeline_model(depth=48, num_redundant_layers=2, **model_kwargs):
    ...
    parts = partition_lb(depth, pipeline_size, num_redundant_layers)[pipeline_rank]
    models = []
    for start, end in parts:
        model_kwargs['first'] = start == 0
        model_kwargs['last'] = end == depth
        model_kwargs['depth'] = end - start
        model = PipelineGPT1D(**model_kwargs).to(get_current_device())
        models.append(model)
        logger.info(f'==> Rank {pipeline_rank} built layer {start}-{end} / total {depth}')
    ...
    for model in models:
        ...
        logger.info(f'Rank{pipeline_rank}/{pipeline_size} model size = {numel * 2 / 1e9} GB')
    # if "checkpoint" in model_kwargs.keys():
    #     if model_kwargs["checkpoint"] is True:
    #         if gpc.get_global_rank() == 0:
    #             assert "checkpoint_path" in model_kwargs.keys(), "You have to specify a file path to use checkpoint loading"
...
def gpt2_8B_lb(**kwargs):
    model_kwargs = dict(dim=3072, depth=72, num_heads=24, **kwargs)
    return _create_lb_gpt_pipeline_model(**model_kwargs)